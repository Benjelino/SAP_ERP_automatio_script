...
//    Fill takes date, internal waybill id, arrival date, date of offloading
      ...
      for(int i = 1; i<count; ++i){
        ...
        Thread.sleep(8000);
        if(data.getRejections()!= 0 && !rejected){
          if(Integer.parseInt(quantity)>data.getRejections()){
            ...
            String addedGrade = "//td[starts-with(@id, '__table') and contains(@id,"+"'" + "_r" + i + "_c7"
                + "'" +")]/div/div/div/input";
            int qauntityLeft = Integer.parseInt(quantity) - data.getRejections();
...
//            jse.executeScript("document.querySelector('table th:last-child').scrollIntoView();");
//            Thread.sleep(5000);
            obj.findElement(By.xpath(addedRow)).sendKeys(String.valueOf(data.getRejections()));
            Thread.sleep(3000);
            ...
            count = count +2;
            rejected=true;
          }else  if(Integer.parseInt(quantity)==data.getRejections()){
            obj.findElement(By.xpath(grade)).clear();
            ...
            obj.findElement(By.xpath(label)).click();
            currentgrade = obj.findElement(By.xpath(grade)).getAttribute("value");
            int rejection = data.getRejections() - Integer.parseInt(quantity);
            data.setRejections(rejection);
          }
        }
  ...
  public void fillOutbound(Data data) throws InterruptedException {
    ...
    for (String[] item: stationnquantity){
      ...
      if (list !=1){
        for(int i=1; i<=list; i++) {
          ...
          obj.findElement(By.xpath(stationmark)).sendKeys(splits[0]);
          Thread.sleep(2000);
          obj.findElement(By.xpath(addedRow)).clear();
          obj.findElement(By.xpath(addedRow)).sendKeys(item[0]);
          obj.findElement(By.xpath("//span[.='Details: Item 10- 1-Cocoa']")).click();
          Thread.sleep(2000);
        }
      }else {
    ...
    Thread.sleep(40000);
    String outboundID = obj.findElement(By.xpath("/descendant::div[@class='sapMMsgStripMessage'][1]/span[1]")).getText();
    data.setOutboundid(outboundID);
  }
